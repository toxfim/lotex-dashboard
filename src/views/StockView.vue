...
const cats = computed(() => [
  "all",
  ...Array.from(new Set(STOCK.map((s) => s.cat))),
]);
...
const rows = computed(() => {
  ...
  return STOCK.filter((i) => cat.value === "all" || i.cat === cat.value)
    .filter(
      (i) =>
        !q ||
...
const kpis = computed(() => [
  {
    label: "Ombor pozitsiyalari",
    val: fmtNum(totals.value.skus),
    sub: "faol SKU",
    icon: "box",
  ...
  },
  {
    ...
    val: compactSom(totals.value.value) + " so'm",
    sub: "tannarxda",
    icon: "coins",
    tint: "good" as const,
  },
  {
    label: "Faol mosliklar",
    val: fmtNum(totals.value.active),
    sub: "navbatdagi lotlar",
    icon: "sparkle",
    tint: "accent" as const,
  },
  {
    label: "Kam qolgan",
    val: fmtNum(totals.value.low),
    sub: "to'ldirish zarur",
    icon: "alert",
    tint: "warn" as const,
  },
]);
      ...
      <div class="page-inner fade-page">
        ...
        <div class="filterbar" style="margin-top: 22px">
          ...
          <span class="result-count"
            ><b class="num">{{ rows.length }}</b> ta pozitsiya</span
          >
        </div>
            ...
            <thead>
              <tr>
                <th class="sortable" @click="toggleSort('name')">
                  <span class="th-in"
                    >Mahsulot<BaseIcon v-if="sortKey === 'name'" name="sort"
                  /></span>
                </th>
                <th>Kategoriya</th>
                <th class="sortable" @click="toggleSort('avail')">
                  <span class="th-in"
                    >Zaxira<BaseIcon v-if="sortKey === 'avail'" name="sort"
                  /></span>
                </th>
                <th class="sortable r" @click="toggleSort('cost')">
                  <span class="th-in"
                    >Birlik narxi<BaseIcon
                      v-if="sortKey === 'cost'"
                      name="sort"
                  /></span>
                </th>
                <th class="sortable r" @click="toggleSort('value')">
                  <span class="th-in"
                    >Qiymati<BaseIcon v-if="sortKey === 'value'" name="sort"
                  /></span>
                </th>
                <th class="sortable" @click="toggleSort('matches')">
                  <span class="th-in"
                    >Faol lotlar<BaseIcon
                      v-if="sortKey === 'matches'"
                      name="sort"
                  /></span>
                </th>
              </tr>
              ...
              <tr v-for="i in rows" :key="i.code" style="cursor: default">
                ...
                <td>
                  ...
                  <div class="avail-bar">
                    <span
                      :style="{
                        width: Math.min(100, (i.avail / i.cap) * 100) + '%',
                        background:
                          i.avail <= i.reorder ? 'var(--warn)' : 'var(--good)',
                      }"
                    />
                  </div>
                </td>
                ...
                <td class="r c-money mono">
                  {{ compactSom(i.avail * i.cost) }}
                </td>
                <td>
                  <span
                    v-if="(matchesByCode[i.code] || 0) > 0"
                    class="matches-link"
                  >
                    <BaseIcon name="sparkle" />{{ matchesByCode[i.code] }} ta
                    lot
                  </span>
